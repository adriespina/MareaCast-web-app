    ...
    >
      ...
      {data?.dataDisclaimer && (
        <div className="bg-amber-900/60 text-amber-100 text-sm px-4 py-2 border-b border-amber-700 text-center">
          ...
          {data.sourceError && (
            <span className="ml-1 text-amber-200/80">({data.sourceError})</span>
          )}
        </div>
      )}
      ...
      {contextMenu.visible && (
        ...
        >
          ...
          <form onSubmit={handleContextSubmit} className="flex flex-col gap-2">
            ...
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-500 text-white text-sm py-1 rounded transition-colors"
            >
              Actualizar
            </button>
          </form>
      ...
      <footer className="p-4 text-center text-xs text-ocean-400">
        <p>
          MareaCast © {new Date().getFullYear()} • Datos del Instituto
          Hidrográfico de la Marina (IHM) y fuentes oficiales españolas
        </p>
      </footer>
    </div>
  );
