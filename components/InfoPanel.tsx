      ...
      <div className="flex-1 flex flex-col justify-center items-center py-8 relative">
        ...
        <div className="absolute inset-0 flex flex-col justify-between opacity-20 pointer-events-none py-12 px-8">
          {[4, 3, 2, 1, 0].map((level) => (
            <div
              key={level}
              className="w-full border-t border-dashed border-white text-xs text-right pr-2"
            >
              {level}m
            </div>
          ))}
        ...
        <div className="relative z-10 text-center">
          <div
            className={`transition-transform duration-700 ${data.isRising ? "-translate-y-2" : "translate-y-2"}`}
          >
            {data.isRising ? (
              <ArrowUp
                size={120}
                className="text-green-500 drop-shadow-[0_0_10px_rgba(34,197,94,0.5)] animate-pulse"
                strokeWidth={1.5}
              />
            ) : (
              <ArrowDown
                size={120}
                className="text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.5)] animate-pulse"
                strokeWidth={1.5}
              />
            )}
          </div>
          <div className="text-6xl font-bold text-white font-mono tracking-tighter mt-2 drop-shadow-xl">
            {data.currentHeight.toFixed(2)}
            <span className="text-2xl text-blue-400 ml-1">m</span>
          </div>
          <div
            className={`text-lg font-bold mt-2 uppercase tracking-widest ${data.isRising ? "text-green-400" : "text-red-400"}`}
          >
