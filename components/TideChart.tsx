...
const getSkyColor = (time: number, sunrise: number, sunset: number): string => {
  ...
  if (t < sunrise - 0.5)
    return interpolateColor(NIGHT, BLUE, (t - (sunrise - 1.5)) / 1);
  ...
  if (t < sunset + 0.75)
    return interpolateColor(GOLD, BLUE, (t - sunset) / 0.75);

  // Blue -> Night (starts 45m after sunset, lasts 1h)
  if (t < sunset + 1.75)
    return interpolateColor(BLUE, NIGHT, (t - (sunset + 0.75)) / 1);
...
export const TideChart: React.FC<TideChartProps> = ({ data }) => {
  ...
  const nextHigh =
    data.tides
      .filter(
        (t) => t.type === "HIGH" && timeToDecimal(t.time) > currentDecimal,
      )
    ...
    >
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          {nextHigh && (
            ...
            >
              <Label
                value={`Pleamar ${nextHigh.height}m`}
          ...
          {nextLow && (
            ...
            >
              <Label
                value={`Bajamar ${nextLow.height}m`}
                position="insideRight"
                fill="#EF4444"
                fontSize={10}
                dy={10}
      ...
      <div className="absolute top-2 right-2 flex flex-col items-end gap-1 text-[10px] text-gray-400 pointer-events-none opacity-80">
        <div className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-yellow-400"></span> Sol
        </div>
        <div className="flex items-center gap-1">
          <span className="w-2 h-0.5 bg-white"></span> Ahora
        </div>
        <div className="mt-1 text-[9px] opacity-70">
          Mueve el cursor para ver el color del cielo
        </div>
      </div>
    </div>
  );
